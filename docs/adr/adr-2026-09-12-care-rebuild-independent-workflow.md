...
- `concurrency.group` は `daily.yml` と同じ `daily-digest` を使う（初版では独立した
  ...
  指摘され修正した）。
- `build-care.mjs` に `--strict` フラグを追加した。`daily.yml` では介護版の失敗が本体の
  生成・公開を止めてはならないため常に exit 0 を返す設計だが、`care-rebuild.yml`
  （介護版単体を再生成する workflow）でこれをそのまま使うと、生成に失敗してもワークフロー
  自体は「成功」のままビルド・デプロイが続行され、記事が更新されていないのに成功したように
  見えてしまう（`codex review` で指摘）。`care-rebuild.yml` は `--strict` 付きで呼び出し、
  失敗時はステップ自体を失敗させて後続のビルド・デプロイを止める。
