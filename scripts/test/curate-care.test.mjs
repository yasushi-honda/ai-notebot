...
// 2026-09-16実データで発覚: 「使えるプロンプト例」内の4項目チェック（見出し・フェンス・識別子・
// 注意書き）がelse ifで連結されていたため、「## 使えるプロンプト例」自身のフェンスが
// 未クローズだと判定された時点で、bodyMarkdown全体を対象にする識別子チェック
// （promptExampleAsksForIdentifyingField、別の既にクローズ済みブロックに識別子があっても
// 検出できるはずの独立したチェック）が同一試行内でスキップされていた。この2問題を同時に
// 持つ出力は「試行1でフェンス指摘→試行2でフェンス直すと識別子が初めて発覚→識別子直すと
// 再びフェンスを壊す」の往復でMAX_REGENERATE_ATTEMPTS（3回）に収まらず再生成が収束しない
// ことがあった（/codex reviewで指摘・修正）。「## 使えるプロンプト例」より前に既に
// クローズ済みの識別子入りブロック（## 補足）を置き、「## 使えるプロンプト例」自身は
// フェンスを開いたままEOFまで閉じない構成にすることで、両問題が同一試行のproblemsに
// 含まれることを保証する。
test('validateGenerated: 「## 使えるプロンプト例」のフェンス未クローズと、別ブロックの識別子問題が同居していれば両方を同一試行で検出する', () => {
  const body =
    '## なぜ手間がかかるのか\n\n背景です[^s-abcdef0123]。\n\n## 手順\n\n' +
    '1. Geminiにステップ1を実行させます[^s-1234567890]。\n' +
    '2. 個人情報を匿名化してからChatGPTに入力します[^s-1234567890]。\n' +
  ...
  assert.ok(problems.some((p) => p.includes('閉じられていない')));
  assert.ok(
    problems.some((p) => p.includes('使えるプロンプト例') && p.includes('コードブロック')),
  );
  assert.ok(problems.some((p) => p.includes('利用者名・施設名等')));
});
