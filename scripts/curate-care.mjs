...
function buildPrompt(items, extraInstructions, researchSummary) {
  ...
  // 「このテーマ案に沿って書く」という指示だけを与えると、テーマ案本文に含まれる未検証の
  // 数値・効果・事例をLLMがそのまま本文に書き写し、そこに（内容とは無関係な）items由来の
  // 正当なidを脚注として付けてしまう恐れがある。citation-gateはid解決の可否のみを検証し、
  // 「その脚注が実際にその文の主張を裏付けているか」までは検証しないため、この経路は
  // 無出典の主張が「検証済み」の体裁で公開されてしまう抜け道になる（2026-09-12、codex
  // reviewで指摘）。そのため「方向性の参考としてのみ使い、内容の根拠にしない」ことを明示する。
  const themeNote = researchSummary
    ? '\n\n## 今回の調査で検討されたテーマの方向性（参考情報。グラウンディングされていない' +
      '自由記述のメモであり、内容の正確性は保証されない）\n' +
      `${researchSummary}\n\n` +
      '上記はあくまで「どの業務領域・どんな切り口で書くか」というテーマ選定の参考情報であり、' +
      '既出記事との重複を避けるために使うこと。上記メモに書かれている具体的な数値・効果・' +
      '事例・固有の言い回しを本文にそのまま書き写したり要約したりしてはならない。本文で' +
      '主張する内容は、必ず後述の「参照可能な出典」内のitemsのみを根拠にすること。'
    : '';
  const extraNote =
