...
function buildPrompt(recentTopics) {
  ...
  return [
    ...
    '- 厚生労働省・自治体・介護保険関連の公的機関、または信頼できるベンダー公式ドキュメント等、',
    '  裏付けとなる公式・信頼できる情報源が存在するテーマを選ぶこと',
...
function buildOfficialFollowupPrompt(researchSummary) {
  return [
    ...
    researchSummary,
    '',
    'この内容の裏付けとなる、厚生労働省・自治体・介護保険関連の公的機関（ドメインが go.jp や lg.jp',
    'など）が公開している一次情報を、Google検索で探してください。ベンダーブログや商用メディアの',
    '記事ではなく、公的機関自身が発行している制度解説・通知・ガイドライン・統計等を優先すること。',
  ].join('\n');
}
